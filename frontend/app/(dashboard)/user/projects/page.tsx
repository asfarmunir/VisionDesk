...
const UserProjectsPage: React.FC = () => {
  ...
  const { data, isLoading, isFetching, refetch, error } = useProjects({
    limit: 100,
  });
  ...
  const memberProjects = useMemo(() => {
    ...
    return list.filter(
      (p) =>
        ...
        p.teamMembers.some((tm) => tm.user._id === user._id)
    );
  }, [data?.projects, user]);
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {displayed.map((p) => (
              <MemberProjectCard
                key={p._id}
                project={p}
                onClick={() => router.push(`/user/projects/${p._id}`)}
              />
            ))}
          </div>
        )}
...
const MemberProjectCard: React.FC<MemberCardProps> = ({ project, onClick }) => {
